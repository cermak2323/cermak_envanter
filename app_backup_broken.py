...
try:
    ...
    print("[WARNING] win32print yüklü değil. Yazıcı desteği devre dışı. (pip install pywin32)")
...
# ============================================
# ZEBRA/TSPL YAZICI FONKSIYONLARI
# ============================================

def center_text_x(text, label_width=400, font_char_width=20, scale_x=1.8):
    """
    TSPL etiketinde metni orta hizaya almak için X koordinatını hesapla
    
    Args:
        text: Yazılacak metin
        label_width: Etiket genişliği (default: 400 dot = 50mm @ 203 DPI)
        font_char_width: Font karakter genişliği (dot)
        scale_x: X ekseni ölçek faktörü
    
    Returns:
        X koordinatı (dot cinsinden)
    """
    text_width = len(text) * font_char_width * scale_x
    x = int((label_width - text_width) / 2)
    return max(x, 0)


def print_label_to_tspl_printer(qr_id, printer_name="4BARCODE 4B-2054TF"):
    """
    TSPL raw data kullanarak Zebra yazıcıya etiket gönder
    
    Args:
        qr_id: QR ID'si (ör: 05686-26600_1)
    ...
    if not WINDOWS_PRINT_AVAILABLE:
        return {
            'success': False, 
            'message': 'win32print kütüphanesi yüklü değil. (pip install pywin32 komutu çalıştır)'
        }
    
    try:
        # Ortalama pozisyonlarını hesapla
        QRID_X = center_text_x(qr_id, label_width=400, font_char_width=15, scale_x=1.2)
        CERMAK_X = center_text_x("CERMAK", label_width=400, font_char_width=20, scale_x=2.0)
        
        # TSPL şablonu - parça kodu küçültüldü (scale 0.6 = 1/3 oranında)
        tspl_template = f"""SIZE 50 mm,50 mm
GAP 2 mm,0
REFERENCE 0,0
DIRECTION 1
CLS
TEXT {CERMAK_X},20,"3",0,2,2,"CERMAK"
QRCODE 60,80,L,12,A,0,"{qr_id}"
TEXT {QRID_X},330,"3",0,0.6,0.6,"{qr_id}"
PRINT 1
"""
PRINT 1
"""
        ...
        try:
            ...
            # TSPL komutlarını gönder
            win32print.WritePrinter(hPrinter, tspl_template.encode("utf-8"))
            
            win32print.EndPagePrinter(hPrinter)
            win32print.EndDocPrinter(hPrinter)
            
            logging.info(f"[PRINTER] {part_code} etiketi {printer_name} yazıcısına gönderildi")
            
            return {
                'success': True, 
                'message': f'{part_code} etiketi {printer_name} yazıcısına gönderildi'
            }
        except Exception as print_error:
            logging.error(f"[PRINTER] Yazdırma işi hatası: {str(print_error)}")
            if hJob:
                try:
                    win32print.EndDocPrinter(hPrinter)
                except:
        ...
        error_msg = f'Yazıcı bulunamadı: {printer_name}'
        logging.error(f"[PRINTER] {error_msg}")
        return {'success': False, 'message': error_msg}
    
    except Exception as e:
        error_msg = f'Yazıcı hatası: {str(e)}'
        logging.error(f"[PRINTER] {error_msg}")
        return {'success': False, 'message': error_msg}
...
def generate_qr_pil_image(qr_id, box_size=8, border=2):
    """QR kod PIL Image olustur - Barkod makinesi icin optimize"""

    qr = qrcode.QRCode(

        version=1,

        box_size=box_size,  # 8px - barkod makinesi icin optimize

        border=border,      # 2px quiet zone - tarayici icin minimum
...
# Rate limiting for IP based tracking
...
def verify_shared_folder_access():
    """Verify shared folder access and permissions"""
    try:
        if not os.path.exists(SHARED_BASE):
            print(f"[ERROR] Shared folder not found: {SHARED_BASE}")
            print(f"[WARN] Linux: '/mnt/ortakdepo' must be mounted")
            return False
...
@app.route('/print_label/<qr_id>', methods=['POST'])
@login_required
def print_label_endpoint(qr_id):
    """Belirtilen QR ID'sini TSPL yazıcıya gönder"""
    try:
        result = print_label_to_tspl_printer(qr_id)
        
        # Başarılı veya başarısız, sonuç geri dön
        return jsonify(result), (200 if result['success'] else 400)
    
    except Exception as e:
        logging.exception(f"[PRINT ENDPOINT] Yazıcı hatası: {e}")
        return jsonify({
            'success': False, 
            'error': f'Yazıcı hatası: {str(e)}'
        }), 500
